...
namespace Eto.UnitTest.UI
{
    public class UnitTestPanel : Panel, INotifyPropertyChanged
    {
        ...
        public UnitTestPanel() : this(true)
        {
        }

        public UnitTestPanel(bool showLog, Orientation logOrientation = Orientation.Horizontal)
        {
            ...
            _tree = new TreeGridView { ShowHeader = false, Size = new Size(400, 300) };
            ...
            if (showLog)
            {
                _log = new TextArea { ReadOnly = true, Wrap = false };

                if (logOrientation == Orientation.Horizontal)
                {
					_log.Size = new Size(400, 300);

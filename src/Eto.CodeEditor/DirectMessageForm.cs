...
namespace Eto.CodeEditor
{
    ...
    internal class MsgListItem : ListItem
    {
        internal MsgListItem() : base()
        {

        }

        public int Msg { get; set; }
        public RetType Rt { get; set; }
        public GS Gs { get; set; }
    }

    internal enum RetType { intT, stringT, boolT, voidT };
    internal enum GS { Get, Set };

    public class DirectMessageForm : Form
    {
        public DirectMessageForm(CodeEditor editor)
        {
            var ta = new TextArea();

            var msgdd = new DropDown();

            Func<int, string, RetType, GS, MsgListItem> f = (i, s, rt, gs) => new MsgListItem { Msg = i, Text = s, Rt = rt, Gs = gs };

            Func<int, RetType, string> retAsStr = (i, rt) => {
                string s = null;
                switch (rt)
                {
                    case RetType.boolT:
                        s = (i == 1 ? true : false).ToString();
                        break;
                    case RetType.intT:
                        s = i.ToString();
                        break;
                    case RetType.stringT:
                        s = "string pointer?";
                        break;
                    default:
                        s = "void";
            ...
            var msgs = new List<MsgListItem>
            {
                // be default 0 which means same as GETTABWIDTH
                f(nm.SCI_GETINDENT, "SCI_GETINDENT", RetType.intT, GS.Get),
                f(nm.SCI_SETINDENT, "SCI_SETINDENT", RetType.voidT, GS.Set),

                f(nm.SCI_GETTABWIDTH, "SCI_GETTABWIDTH", RetType.intT, GS.Get),
                f(nm.SCI_SETTABWIDTH, "SCI_SETTABWIDTH", RetType.voidT, GS.Set),
                //f(nm.SCI_GETTABMINIMUMWIDTH, ) nope
                f(nm.SCI_GETUSETABS, "SCI_GETUSETABS", RetType.boolT, GS.Get),
                f(nm.SCI_SETUSETABS, "SCI_SETUSETABS", RetType.voidT, GS.Set),

                f(nm.SCI_GETTABINDENTS, "SCI_GETTABINDENTS", RetType.boolT, GS.Get),
                f(nm.SCI_SETTABINDENTS, "SCI_SETTABINDENTS", RetType.voidT, GS.Set),

                f(nm.SCI_GETBACKSPACEUNINDENTS, "SCI_GETBACKSPACEUNINDENTS", RetType.boolT, GS.Get),
                f(nm.SCI_SETBACKSPACEUNINDENTS, "SCI_SETBACKSPACEUNINDENTS", RetType.voidT, GS.Set)
            };
            msgdd.Items.AddRange(msgs);

            var btn = new Button { Text = "Sci Direct" };
            btn.Click += (s, e) =>
            {
                var si = (msgdd.SelectedValue ?? msgdd.Items.First()) as MsgListItem;
                int.TryParse(ta.Text, out var i);
                int r = editor.direct_message(si.Msg, i);
                ta.Text = retAsStr(r, si.Rt);
            };
            ...
            Content = new TableLayout
            {
                Rows =
                {
                    new TableRow {Cells = {ta, msgdd, btn} }
                }
            };
        }
    }
